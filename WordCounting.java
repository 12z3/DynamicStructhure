...
//                "In this lesson we take a look at lists and their application!";
    ...
    private static void countingWordsInTextC(String text) {
        ...
        int countThisWord, j = 0;

        String[] words = text.trim().split(" ");
        while (j < words.length) {
            thisWord.append(words[j]);
            if (wordsCounts.containsKey(thisWord.toString())) {           // Има ли ключ съответстващ на тази дума?
                countThisWord = wordsCounts.get(thisWord.toString());     // ... дай ми на тази дума повторенията(Keys)
                countThisWord++;
            } else {
                countThisWord = 1;
                //System.out.println(thisWord);
            }
            wordsCounts.put(thisWord.toString(), countThisWord);          //  Обновява Map-a
    ...
    // Обхожда масива с думи една по-една и за всяка дума пита дали се съдържа в "wordsCounts".
    // Ако се съддържа увеличава брояча й с 1. Ако ли не задава брояча й на 1.
    // Първоначално за всяка бройка "countThisWord" на дадена дума  Key-й "currentWord" = 0;
    private static void countingWordsInTextCA(String text) {
        ...
        while (j < words.length) {
            ...
            wordsCounts.put(currentWord, countThisWord);            //  Обновява Map-a
            j++;
        }
        for (Map.Entry<String, Integer> el : wordsCounts.entrySet()) {
            System.out.printf("%s -> %d%n", el.getKey(), el.getValue());
