...
public class eFinance {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        findByType(allData, "BREZOVO");
        findByType(allData, "ТАКСА ПАКЕТНО ОБСЛУЖВАНЕ");
        //findByType(allData, "ТРАНСФЕР");
        //printListDataTmp(allData, 66, 2);

        //findByDate(allData, "31.12.2022", "30.04.2024");
        findByDate(allData, "31.12.2023", "07.06.2024");
            ...
            while (scanner.hasNextLine()) {
                ...
                if (cnt > 3 && !lineWord[0].contains(" ,")) {
                    if (!lineWord[0].isEmpty() && matcher.find()) {
                        ...
                        String res = matcher.replaceAll("-");              // заменя намерените "," с "-"
    ...
    // todo: Oт дата 31.03.2024 до дата 30.04.2024: <- Това са напълно валидни дати - 404: Оправи си датите ;)
    static boolean dateBoundsIsNotValid(List<String> list, int fromYear,
        ...
        // Това е за случая в който годините съвпадат, а когато не съвпадат?
        if (fromYear == toYear) {
            return fromYear < Math.min(fromTmpYear, toTmpYear) || (fromTmpYear > Math.max(fromTmpYear, toTmpYear));
        } else {
...
//                || ((fromMonth > fromTmpMonth) || (toMonth > toTmpMonth));
        ...
        //Oт дата 31.03.2024 до дата 30.04.2024: 31.03.2024 е невалидна дата
//        return ((fromYear > fromTmpYear) && (toYear > toTmpYear))
//                || ((fromMonth > fromTmpMonth) && (toMonth > toTmpMonth));
    }
    ...
    static double[] getIdxByDate(List<String> list, String fromDate, String toDate) {
        ...
        // Проверката е за валиден брой цифри в датите: 10.06.2024
        try {
            fromDayInt = Integer.parseInt(fromDate.substring(0, 2));              // 10
            toDayInt = Integer.parseInt(toDate.substring(0, 2));
            fromMonthInt = Integer.parseInt(fromDate.substring(3, 5));            // 06
            toMonthInt = Integer.parseInt(toDate.substring(3, 5));
            fromYearInt = Integer.parseInt(fromDate.substring(6));     // 2024
        ...
        if (dateBoundsIsNotValid(list, fromYearInt, fromMonthInt, toYearInt, toMonthInt)) {
            System.out.println(
        ...
        // Така върви хронологията във файла
        ...
        for (int i = 0; i < list.size(); i++) {
            matcherFrom = pattern1.matcher(list.get(i).trim());
            matcherTo = pattern2.matcher(list.get(i).trim());
            // За този ред иде реч по-горе.
            if (!fromDateIdxIsFind && matcherFrom.find()) {
                cnt1++;
                fromIdx = i;
                fromDateIdxIsFind = true;
            } else if (fromDateIdxIsFind && matcherTo.find()) {
                cnt2++;
                toIdx = i;
            }
        }
    ...
    static void findNextValidDate2(List<String> list, String date) {
        ...
        // Милионите проверки гарантират, че дата ще я има и while цикъла няма да е безкраен. - Рисковано е.
        while (searchingMatchesDates(list, date) == -1) {
            String tmpStr = date.substring(2);
            String tmpStrM = date.substring(5); // todo: Here was 5
            String tmpDate = null;


            if (intDay >= 9 && intDay < 31) {
                intDay++;
                tmpDate = intDay + tmpStr;
            } else if (intDay > 0 && intDay < 9) {
                intDay++;
    ...
    static int searchingMatchesDates(List<String> list, String date) {
        ...
        for (int i = 0; i < list.size(); i++) {
            matcher = pattern.matcher(list.get(i).trim().split("-")[0]);
            if (matcher.find()) {
                res = matcher.group();
    ...
    static int getTransaction(List<String> list, int line, boolean isPrint) {
        ...
        if (isPrint && !dataTmp[1].isEmpty()) {
            System.out.print(dataTmp[1] + " " +dataTmp[2]+ " ");
        }
        return dataTmp[1].length() + dataTmp[2].length() + 1;
    }
