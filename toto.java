...
public class toto {
    ...
    private static void play(List<Integer> officialResult, int matches) {
        ...
        System.out.println("\nAll available attempts are: " + attempts + " for " + matches + " digits");
    }
    ...
    private static int checkMatches(List<List<Integer>> res, List<Integer> officialResult, int matches) {
        for (List<Integer> item : res) {
            int tmp = checkResults(officialResult, item);
            if (tmp == matches) {
                System.out.println();
                printResult(res);
                return tmp;
